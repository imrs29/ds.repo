...
void explainVector() {
	...
	v.push_back(1); //{1} // push back and emplace back are same
	...
	vector<int>::iterator it = v.end(); //it will point to the memory address after 40 not 40 -->null memory address

	vector<int>::iterator it = v.rend(); ////it will points to memory address b4 10 b4 the begin

	vector<int>::iterator it = v.rbegin(); //it will point to 40 not after 40 point to exactly 40 


	// cbegin(), cend();

	cout << v[0] << " " << v.at(0); // both same

	cout << v.back() << " "; // to get the size of element
